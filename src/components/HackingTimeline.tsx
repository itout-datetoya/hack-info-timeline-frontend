...
import { Tag, Incident } from '../types';
  ...
  useEffect(() => {
    const fetchTags = async () => {
      try {
        ...
        setError('タグの読み込みに失敗しました。サーバーが起動しているか確認してください。');
      }
    };
    fetchTags();
  }, []);
    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="space-y-6">
          {incidents.slice(0, displayedCount).map(incident => (
            <IncidentCard key={incident.id} incident={incident} />
          ))}
        </div>
        {incidents.length > displayedCount && (
          <div className="text-center mt-8">
            <button onClick={handleLoadMore} className="bg-gray-700 hover:bg-gray-600 text-gray-300 font-bold py-2 px-6 rounded-lg transition-colors duration-200">
              さらに表示
            </button>
          </div>
        )}
